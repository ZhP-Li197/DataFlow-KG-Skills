...
| 6 | KG 算子类别 | GENERAL_KG（Recommended，覆盖最广）/ TEMPORAL_KG / GRAPH_RAG / HRKG / 领域KG（FinKG、MedKG 等） | `KG_Category` |
...
| 5 | DataFrame 列名映射 | 使用矩阵默认列名（Recommended，如 `raw_chunk→entity→triple`）/ 自定义列名 | `run_input_keys`, `run_output_key` |
...
- `OP_NAME` 不在 `operator-compatibility-matrix.md` 中，且 `run_input_keys` 未提供
- 用户提供的 `run_output_key` 与矩阵规范冲突（如声明 `entity_type` 但矩阵显示应为 `entity`）
...
- 若 OP_NAME 在 `operator-compatibility-matrix.md` 中，直接使用矩阵填充 Stage 1 接口字段，无需追问接口细节。
- 若 OP_NAME 不在矩阵中，Round 2 的 Block 5 必须触发（追问 `run_input_keys` 和 `run_output_key`）。
